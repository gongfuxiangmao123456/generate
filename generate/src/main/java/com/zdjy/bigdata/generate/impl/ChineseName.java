 ...
 * 
 * 姓：从百家姓中随机取出来一个
 * 名：随机汉字，通过构造方法传入名字的长度
 * 
 * 
 * @author david
 * @create 2017年11月19日 下午9:45:00
 */
public class ChineseName extends Chinese implements AbstractGenerater<String>{
	...
	private int secondNameSize;
	private FieldSurround fieldSurround;
	public ChineseName(SecondNameSizeEnum secondNameSizeEnum,FieldSurround fieldSurround) {
		this.secondNameSize=secondNameSizeEnum.size;
		this.fieldSurround=fieldSurround;
	}
	public ChineseName(FieldSurround fieldSurround) {
		this(SecondNameSizeEnum.TWO,NullSurrond.getInstance);
	}
	public ChineseName(SecondNameSizeEnum secondNameSizeEnum) {
		this(secondNameSizeEnum,NullSurrond.getInstance);
	}
	public ChineseName() {
		this(SecondNameSizeEnum.TWO);
	}
	@Override
	public String generate() {
		String temp=firsname[RANDOM.nextInt(firsname.length)];
		for(int i=0;i<secondNameSize;i++) {
			temp+=super.generate();
		}
		return fieldSurround.surrond(temp);
	}
	
	/**
	 * 名字长度枚举类
	 * @author david
	 * @create 2017年11月19日 下午9:50:34
	 */
	enum SecondNameSizeEnum{
		ONE(1),TWO(2),THREE(3),FOUR(4);
		private int size;
		private SecondNameSizeEnum(int size) {
			this.size=size;
		}
		public int getSize() {
			return size;
